...
test("2. the route directory holds EXACTLY the six approved files", () => {
  const routeFiles = [
    ...new Set([
      ...gitLines(["ls-files"]),
...
test("5. the module exports EXACTLY ONE action, with the exact signature", () => {
  const exported = [
    ...ACTIONS_SOURCE.matchAll(/export (?:async )?function (\w+)\(/g),
  ].map(([, name]) => name);
  assert.deepEqual(exported, ["createExamDefinitionAction"]);
...
test("9. the action calls the committed writer with the bound id and the raw input", () => {
  ...
  // The writer module is the ONLY mutation import.
...
test("10. offering_not_found leaves this route for the course list", () => {
  ...
  const destination = ACTIONS.slice(ACTIONS.indexOf('"/admin/courses?error=invalid"'));
  assert.equal(
...
test("11. success revalidates ONLY this exams path and redirects with the flag", () => {
  ...
  assert.equal((ACTIONS.match(/revalidatePath\(/g) ?? []).length, 1);
  assert.ok(ACTIONS.includes("revalidatePath(examsPath);"), "the wrong path is revalidated");
  assert.ok(
    ACTIONS.includes("redirect(`${examsPath}?createdDefinition=1`);"),
  ...
  // Revalidation happens ONLY on success.
  assert.ok(
    ACTIONS.indexOf("if (result.ok)") < ACTIONS.indexOf("revalidatePath("),
...
test("13. there is NO try/catch, so every redirect is outside one", () => {
  ...
  assert.equal((ACTIONS.match(/redirect\(/g) ?? []).length, 4);
});
...
test("15. no edit, delete, reorder, plan, session, source-date or publication path exists", () => {
  for (const token of [
...
test("30. the slice touched EXACTLY its eight approved paths", () => {
