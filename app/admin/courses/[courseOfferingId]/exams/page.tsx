/**
 * EXAM EX-S5B-5B + EXAM PLAN P3 + EXAM EX-S5B-5C + EXAM EX-SES-UI-1 — the admin
 * Exams surface of ONE course offering: a read of its ExamDefinition
 * configuration AND of its scheduled exam sessions, plus the THREE explicit
 * create affordances that belong to it.
 *
 ...
 * reach are the three Server Actions bound below.
 *
 * ===========================================================================
 * WHAT THIS ROUTE MAY MUTATE — AND WHAT IT STILL MAY NOT
 * ===========================================================================
 * EXACTLY THREE mutations exist here. The first two are MUTUALLY EXCLUSIVE by the
 * state of the plan; the third additionally requires something to schedule:
 *
 ...
 *
 * Editing, removing and reordering definitions, EDITING or REMOVING a session,
 * reordering sessions, deleting or publishing the plan, assignments, breaks,
 * supervisors and source dates are NOT reachable — not disabled, not hidden
 * behind a flag, but absent, with no import that could reach them.
 *
 * All three creates are ALWAYS an explicit click. The page performs no write, so a
 * plain GET of this route — a refresh, a back button, a prefetch, a bookmark —
 * can never bring a plan, a definition or a session into existence. There is no
 * effect, no auto-submit and no redirect that writes.
 *
 ...
 * END time (this surface derives no duration), the session and definition ids,
 * the exam kind, the sequence number, the version stamp, waves, slots, capacity
 * and every assignment detail. The assignment COUNT is shown as explanatory text
 * only; the writers, not this page, remain the authority on what that count
 * permits. Every date the plan holds is listed, earliest first, including past
 * ones: this is a configuration surface, not a "what is next" view, so it reads
 * no clock and filters against none.
 *
