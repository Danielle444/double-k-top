 ...
 * chronological sub-view.
 *
 * Calls ONLY the two unified readers - no course offering id, no instructor id,
 * no eligibility flag is ever sent. Renders every already-authorized item they
 * return as a plain chronological STACKED list, deliberately NEVER
 * ScheduleTimeGrid: that grid lays items out in fixed group-א / group-ב
 * columns for ONE course, a model that cannot represent two offerings
 * side-by-side (and whose contiguous-same-title coalescing would merge items
 * across offerings). Every item's time/title/location/note/instructor/
 * riding-click-through/compact-details-modal behaviour is reused verbatim from
 * the EXISTING InstructorScheduleCard - no card, no server logic and no
 * filtering is duplicated here.
 *
...
import type { InstructorScheduleFilter } from "@/lib/actions/instructor-schedule-course-scoped";
...
export function UnifiedInstructorScheduleSection({
  ...
  // Reports the SELECTED WEEK's date range (never a course) upward, exactly as
  // the per-course week browser already does, so the shared riding-activity map
  // keeps working without this view holding any course state.
  ...
  // Defaults to the full schedule, matching the per-course section's own default.
  ...
  // Publish the selected week's range upward, and CLEAR it on unmount - which
  // is what leaving the unified sub-view causes - so the shared riding-activity
  // map can never resolve a card against a stale range.
  useEffect(() => {
  ...
  useEffect(() => {
    // Nothing is requested until a merged week is selected.
    ...
    getUnifiedScheduleForInstructor(rangeStart, rangeEnd, dayFilter, scheduleFilter)
      .then((result) => {
        if (cancelled) return;
        if (!result.eligible) {
  ...
  }, [rangeStart, rangeEnd, dayFilter, scheduleFilter]);
        ...
        <>
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-xl font-bold text-card-foreground">הלו&quot;ז המשולב שלי</h2>
            ...
            {!rangeStart || !rangeEnd || itemsState.status === "loading" ? (
              <p className="text-base text-muted-foreground">טוען...</p>
              ...
              // Chronological STACKED list only - never ScheduleTimeGrid, and
              // never re-grouped or re-sorted here: the pure core
              // (mergeUnifiedInstructorScheduleSources) already returns items
              // in one global chronological order across both offerings.
                    ...
                    extraBadges={
                      <>
                        {/* Always-visible source-course badge - the whole point
                            of the merged list is knowing which course an item
                            came from, so it is never hidden behind the
                            compact-details dialog. */}
                        <span className="rounded-full bg-primary px-2 py-0.5 text-xs font-medium text-primary-foreground">
                          {item.sourceCourseLabel}
                        </span>
                        {/* Minimal cross-offering overlap indicator: a visible
                            badge only, never conflict resolution and never a
                            link to the other course. */}
